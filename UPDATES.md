...
### 2026-08-30 — Sprint 11: Almasse `electricite_generale` + S11 polish + Desktop exe (`9709e17`, CI 1m00s)

- **What:** Built Almasse as a first-class DSC project — feasibility for a store of électricité générale with Almasse inventory/POS. Added `BUSINESS_TEMPLATES["electricite_generale"]` (1.5–6M DZD, 18–30% margin, 2–5 staff, 40–120m²; products câbles/disjoncteurs/prises/LED + Almasse barcode/réassort/POS) + `REGULATORY_CHECKLISTS["electricite_generale"]` (CNRC 607-003, IANOR, habilitation, CNAS/CASNOS). Sector pack `ELECTRICITE_GENERALE.md` (391 lines, §1–10 mirror `DIGITAL_SERVICES.md`: marché El Bayadh 0.65, grille DZD, flux `accueil→devis→stock Almasse→POS→facture→réassort→inventaire`, invest ~2.75M, RH SNMG 24K, VAN@12%/seuil/TRI, 5 risques, plan 30/60/90) + gallery `electricite_elbayadh.html` (268 lines, 9 sections, KPI VAN/TRI/seuil, AAPI 495/1500) as 4th card. S11: `Idempotency-Key` header on `POST /billing/checkout` + `POST /v1/dossiers/feasibility` (in-mem `_CHECKOUT/FEASIBILITY_IDEMPOTENCY` dicts, TODO persist), error envelope (`422 validation_error` + `500 internal_error` with `HTTPException` passthrough), `has_more`/`pages` on `GET /v1/dossiers/`. Desktop: `desktop_app.py` port fallback `8000→8001→8002` (`socket.bind` probe), `dsc.spec` `bidi` fix (was `python_bidi`) + `feasibility/sectors` explicit + 6 hiddenimports (`offline_templates`/`financial_calculators`/`nesda_calculator`/`policy_constants`/`dsc_utils`/`feasibility_generator`). `pages/feasibility.py` dropdown is dynamic (`list(BUSINESS_TEMPLATES.keys())`) — electricite auto-appears.
- **Build:** `pyinstaller dsc.spec --clean → dist/DSC_Digital_Services_Center.exe` **182 MB**, zero hidden-import errors (previous `python_bidi not found` fixed). Offline smoke `feasibility_offline('electricite_generale','Almasse El Bayadh','El Bayadh','32-El Bayadh',3000000)` → 10.6K Arabic md, `offline True`; `calculate_real_financials(3M, electricite_generale, 32-El Bayadh)` → 3 scenarios + compte/tresorerie/bilan. Landing stays server, 8 routes client, `not-found/error/loading` from `04fc1dc` still present.
- **Files:** `feasibility_generator.py` (+18), `feasibility/sectors/ELECTRICITE_GENERALE.md` (new), `gallery/electricite_elbayadh.html` (new), `gallery/index.html` (4th card), `apps/api/app/{main,billing,dossiers}` (S11), `desktop_app.py` (+socket), `dsc.spec` (+datas/hiddenimports).
- **Verification:** `pytest 164/4 · verify_rates --strict 67/67 · check_public_site OK · py_compile 5/5 · spec ast OK · exe 182MB`. CI `33336855340` 1m00s green.
- **Note on financials:** `calculate_real_financials(3M @ 24% marge × 0.65)` gives VAN ≈ −3.1M (TRI clamped 1000) — honest for a small El Bayadh store barely above seuil (honest ≠ bug; policy_constants VAN 12%/CNAS 34.5% are correct per 7-lane math audit). Gallery frames it as conditional viability (needs >5M CA or LED mix 28-30%); tune `investment` to 4-5M or wilaya `16-Alger 1.15` for strongly positive VAN when pitching AAPI.
- **How to run Almasse:** `python desktop_app.py` (or double-click `dist/DSC_Digital_Services_Center.exe`) → browser `http://localhost:8000` → `Feasibility` → select `electricite_generale` → wilaya `32-El Bayadh` (or your wilaya), investment 3-4M → `Generate` → `Export PDF` / `gallery/electricite_elbayadh.html` as committee sample. Offline needs no key; LLM lane uses `llama-3.3-70b` if `GROQ_API_KEY` set.
