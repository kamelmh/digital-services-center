...
2. ``make_rate_limit(spec)`` — dependency-based sliding-window limiter.
   ...
   In-memory per-process bucket keyed by client IP + path. With multiple
   workers each enforces its own window (effective limit = spec x workers) —
   acceptable for the current Render 2-worker deployment.
"""
import threading
import time
