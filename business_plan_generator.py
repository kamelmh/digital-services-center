...
class BusinessPlanGenerator:
    ...
    def generate(
        ...
        if getattr(self, "offline", False) or not getattr(self, "api_key", None):
            ...
            result = business_plan_offline(business_type, business_name, location, wilaya, investment)
            try:
                from quality_scorer import QualityScorer as _QS0
                _qr0 = _QS0().score("business_plan", result["content"])
            ...
            try:
                ...
                hook_generation(generator="business_plan", input_params={"business_type": business_type, "business_name": business_name, "location": location, "wilaya": wilaya, "investment": investment, "mode": "offline"}, output_content=result["content"], metadata={"sections": list(result["sections"].keys()), "offline": True, "prompt_version": getattr(self, "prompt_version", "unknown"), **_qm0})
            except Exception:
                pass
            return result
...
**تاريخ الإعداد:** """ + now.strftime("%d/%m/%Y")
        ...
        try:
            ...
            _qr = _QS().score("business_plan", content)
